...
struct HinhDang
{
    char a[3][3]; // Hình dạng của Fighter và Enemy
    char b[3][1]; // Hình dạng của đạn lever 2
};
...
void KhoiTaoDanhSachEnemy3(DsEnemy &dse3)
{
    //Khởi tạo có 100 tàu địch và tất cả đang trong trạng thái OFF
    dse3.n = 50;
    dse3.START = 0;
    ...
    //Khởi tạo hình dáng cho 100 tàu
    for(int i = 0; i < dse3.n; i++)
    {
        //Trạng thái chưa xuất hiện
        dse3.e[i].tt = KTT;

        //Khởi tạo 2 viên đạn của Enemy 3
        dse3.e[i].v1.tt = OFF;
        dse3.e[i].v2.tt = OFF;
        ...
        // Khởi tạo trạng thái
        dse3.e[i].checkUp = false;
        dse3.e[i].checkDown = true;

        dse3.e[i].hd.a[0][0] = ' ';
        dse3.e[i].hd.a[1][0] = '<';
        dse3.e[i].hd.a[2][0] = ' ';
        dse3.e[i].hd.a[0][1] = 201;
        dse3.e[i].hd.a[1][1] = 206;
        dse3.e[i].hd.a[2][1] = 200;
        dse3.e[i].hd.a[0][2] = ' ';
        dse3.e[i].hd.a[1][2] = 246;
        dse3.e[i].hd.a[2][2] = ' ';
    }
}
...
void KhoiTaoDan1(DsDan1 &dd1){
    dd1.n = 40; //Khởi tạo tàu địch ban đầu có 20 viên đạn.
    dd1.START = 0;
    dd1.END = 0;
...
void KhoiTaoDan2(DsDan2 &dd2){
    dd2.n = 25; //Khởi tạo ten lua ban đầu có 10 viên đạn.
    dd2.START = 0;
    dd2.END = 0;
    ...
    for(int i = 0; i < dd2.n; i++)
    {
        ...
        dd2.d2[i].hd.b[0][0] = 220;
        dd2.d2[i].hd.b[1][0] = ' ';
        dd2.d2[i].hd.b[2][0] = 223;
    }
}

void KhoiTaoDan3(DsDan3 &dd3){
    dd3.n = 15; //Khởi tạo ten lua ban đầu có 5 viên đạn.
    dd3.END = 0;
...
void HienThiKhung()
{
    for(int i = 0; i < ConsoleHeightGame; i++)
    {
        for(int j = 0; j < ConsoleWidth; j++)
        {
            if(i == 0 || i == ConsoleHeightGame - 1)
                Draw_a_Box(i, j, 219, 9);
        }
    }
    for(int i = ConsoleHeightGame; i < ConsoleHeight; i++)
        Draw_a_Box(i, 11, 219, 9);
}
...
void HienThiTauDich3(DsEnemy &dse3)
{
    for(int k = dse3.START; k <= dse3.END; k++)
    {
        if(dse3.e[k].tt == ON)
        {
            for(int i = -1; i <= 1; i++)
            {
                for(int j = -1; j <= 1; j++)
                {
                    Draw_a_Box(dse3.e[k].td.y + i, dse3.e[k].td.x + j, dse3.e[k].hd.a[i + 1][j + 1], 13);
                }
            }
        }
        if(dse3.e[k].td.x == 1)
...
void HienThiDan1(DsDan1 &dd1, DsEnemy &dse3)
{
    ...
    for(int i = dse3.START; i <= dse3.END; i++)
    {
        //Viên đạn số 1
        if(dse3.e[i].v1.tt == ON)
        {
            Draw_a_Box(dse3.e[i].v1.td.y, dse3.e[i].v1.td.x, 4, 13);
        }
        if(dse3.e[i].v1.td.x <= 1)
        {
            dse3.e[i].v1.tt = OFF;
            dse3.e[i].v1.td.x = -1;
            dse3.e[i].v1.td.y = -1;
        ...
        //Viên đạn số 2
        if(dse3.e[i].v2.tt == ON)
        {
            Draw_a_Box(dse3.e[i].v2.td.y, dse3.e[i].v2.td.x, 4, 13);
        }

        if(dse3.e[i].v2.td.x <= 1)
        {
            dse3.e[i].v2.tt = OFF;
            dse3.e[i].v2.td.x = -1;
            dse3.e[i].v2.td.y = -1;
...
void HienThiDiem(DsDan1 dd1, DsDan2 dd2, DsDan3 dd3)
{
    string pDiem = "DIEM : ";
    stringstream ss;
    ss << diem;
    ...
    for(int i = 0; i < pDiem.length(); i++)
        Draw_a_Box(27, 1 + i, pDiem[i], 11);

    //Số lượng đạn 1 :
    Draw_a_Box(27, 15, 4, 10);
    Draw_a_Box(27, 16, ':', 10);
    char d1[5];
    itoa(dd1.n - dd1.END, d1, 10);
    for(int i = 0; i < strlen(d1); i++)
        Draw_a_Box(27, 17 + i, d1[i], 10);

    //Số lượng đạn 2 :
    Draw_a_Box(26, 34, 220, 9);
    Draw_a_Box(28, 34, 223, 9);
    Draw_a_Box(27, 36, ':', 9);
    char d2[5];
    itoa(dd2.n - dd2.END, d2, 10);
    for(int i = 0; i < strlen(d2); i++)
        Draw_a_Box(27, 37 + i, d2[i], 9);

    //Số lượng đạn 3 :
    Draw_a_Box(27, 60, 219, 13);
    Draw_a_Box(27, 61, 219, 13);
    Draw_a_Box(27, 62, 219, 13);
    Draw_a_Box(27, 63, 219, 13);
    Draw_a_Box(27, 64, ':', 13);

    char d3[5];
    itoa(dd3.n - dd3.END, d3, 10);
    for(int i = 0; i < strlen(d3); i++)
        Draw_a_Box(27, 65 + i, d3[i], 13);

}

void DisPlay(Fighter &ft, DsEnemy &dse,DsEnemy &dse2, DsEnemy &dse3, DsDan1 &dd1, DsDan2 &dd2, DsDan3 &dd3)
{
    HienThiDiem(dd1, dd2, dd3);
    HienThiKhung();
    HienThiTenLua(ft);
    HienThiTauDich(dse);
    HienThiTauDich2(dse2);
    HienThiTauDich3(dse3);
    HienThiDan1(dd1, dse3);
    HienThiDan2(dd2);
...
void ConTrol(Fighter &ft, DsEnemy &dse, DsEnemy &dse2, DsEnemy &dse3, DsDan1 &dd1, DsDan2 &dd2, DsDan3 &dd3)
{
    ...
    if(_kbhit())
    {
        ...
        if((key == 'W' || key == 'w') && ft.td.y > 2)
            ft.td.y--;
        else if((key == 'S' || key == 's') && ft.td.y <= ConsoleHeightGame - 4)
            ft.td.y++;
    ...
    if(Flag % 20 == 0)
    {
        ...
        while(k != 0)
        {
            ...
            dse.e[dse.END].td.y = 2 + rand() % (ConsoleHeightGame - 4);
            dse.e[dse.END].tt = ON;
            dse.END++;
            k--;
        }
    ...
    //Xu ly sinh tau địch 2

    if(Flag % 47 == 0 && Flag >= 50)
    {
        int k = 1;
        while(k != 0)
        {
            dse2.e[dse2.END].td.x = ConsoleWidth - 2;
            dse2.e[dse2.END].td.y = 2 + rand() % (ConsoleHeightGame - 4);
    ...
    for(int i = dse2.START; i < dse2.END; i++)
    {
        if(dse2.e[i].tt == ON && dse2.e[i].td.x >= 1)
        {
            dse2.e[i].td.x--;
            int k = -1 + rand() % 3;
            if(dse2.e[i].td.y + k > 1 && dse2.e[i].td.y + k < ConsoleHeightGame - 2)
                dse2.e[i].td.y += k;
        }
    }
    ...
    //Sinh tàu địch 3
    if(Flag % 77 == 0 && Flag > 150)
    {
        int k = 1;
        while(k != 0)
        {
            //Khởi tạo tọa độ ngẫu nhiên cho địch
            dse3.e[dse3.END].td.x = ConsoleWidth - 2;
            dse3.e[dse3.END].td.y = 3 + rand() % (ConsoleHeightGame - 6);

            //Tọa độ gốc lấy làm tâm cho tàu 3 di chuyển quanh trục của nó
            dse3.e[dse3.END].tdg.y = dse3.e[dse3.END].td.y;

            //Trạng thái
            dse3.e[dse3.END].tt = ON;

            //Thời điểm bắn đạn 3
            dse3.e[dse3.END].t1 = 20 + rand() % (50 - 20 + 1);
            dse3.e[dse3.END].t2 = 51 + rand() % (ConsoleWidth - 3 - 51 + 1);
    ...
    //Xử lý tàu dich 3 di chuyen
    for(int i = dse3.START; i <= dse3.END; i++)
    {
        if(dse3.e[i].tt == ON)
        {
            dse3.e[i].td.x--;
            if(dse3.e[i].checkDown == true && dse3.e[i].checkUp == false)
                dse3.e[i].td.y++;
            else if(dse3.e[i].checkDown == false && dse3.e[i].checkUp == true)
                dse3.e[i].td.y--;

            if(dse3.e[i].td.y == dse3.e[i].tdg.y + 1)
            {
                dse3.e[i].checkDown = false;
                dse3.e[i].checkUp = true;
            }
            else if(dse3.e[i].td.y == dse3.e[i].tdg.y - 1)
            {
                dse3.e[i].checkDown = true;
                dse3.e[i].checkUp = false;
            }
            ...
            //Xử lý tàu 3 bắn đạn
            if(dse3.e[i].td.x == dse3.e[i].t1)
            {
                dse3.e[i].v1.tt = ON;
                dse3.e[i].v1.td.x = dse3.e[i].td.x - 1;
                dse3.e[i].v1.td.y = dse3.e[i].td.y;
            }
            if(dse3.e[i].td.x == dse3.e[i].t2)
            {
                dse3.e[i].v2.tt = ON;
                dse3.e[i].v2.td.x = dse3.e[i].td.x - 1;
                dse3.e[i].v2.td.y = dse3.e[i].td.y;
            }

            //Xử lý viên đạn di chuyển
            if(dse3.e[i].v1.tt == ON)
                dse3.e[i].v1.td.x -=2;
            if(dse3.e[i].v2.tt == ON)
                dse3.e[i].v2.td.x -=2;
    ...
    //Xu lý tăng dần START
    if(dse3.e[dse3.START].tt == OFF)
        dse3.START++;
}

int Process(Fighter ft, DsEnemy &dse, DsEnemy &dse2, DsEnemy &dse3, DsDan1 &dd1, DsDan2 &dd2, DsDan3 &dd3)
{
    Flag++;
    ...
    for(int i = dse3.START; i <= dse3.END; i++)
    {
        if(dse3.e[i].tt == ON)
        {
            ...
            for(int j = dd1.START; j <= dd1.END; j++)
            {
                int dX = abs(dse3.e[i].td.x - dd1.d1[j].td.x);
                int dY = abs(dse3.e[i].td.y - dd1.d1[j].td.y);
                if(dX <= 1 && dY <= 1)
                {
                    dd1.d1[j].tt = OFF;
                    dd1.d1[j].td.x = -1;
            ...
            for(int k = dd2.START; k <= dd2.END; k++)
            {
                int dX = abs(dse3.e[i].td.x - dd2.d2[k].td.x);
                int dY = abs(dse3.e[i].td.y - dd2.d2[k].td.y);
                if(dX <= 1 && dY <= 2)
                {
                    dd2.d2[k].tt = OFF;
                    dd2.d2[k].td.x = -1;
            ...
            if(abs(dse3.e[i].td.y - dd3.d3[dd3.END].td.y) <=1 && dse3.e[i].td.x > ft.td.x)
                dse3.e[i].HP--;

            //Xử lý khi máu tàu địch = 0
            if(dse3.e[i].HP <= 0) // Nếu tàu địch hết máu mà vẫn đang ON
            {
                dse3.e[i].tt = OFF;
                dse3.e[i].td.x = -10;
                dse3.e[i].td.y = -10;
            ...
            {
                //Xử lý va chạm
                int dX = abs(ft.td.x - dse3.e[i].td.x);
                int dY = abs(ft.td.y - dse3.e[i].td.y);
                if(dX < 3 && dY < 3)
                    return -1;

                //Xử lý đạn bắn trúng tàu địch
                //Đạn 1
                int ddX1 = abs(ft.td.x - dse3.e[i].v1.td.x);
                int ddY1 = abs(ft.td.y - dse3.e[i].v1.td.y);
                //Đạn 2
                int ddX2 = abs(ft.td.x - dse3.e[i].v2.td.x);
                int ddY2 = abs(ft.td.y - dse3.e[i].v2.td.y);

                if((ddX1 <= 1 && ddY1 <= 1) || (ddX2 <= 1 && ddY2 <= 1))
                    return -1;
            }
        }
        else
...
int main()
{
    ...
    while(1)
    {
        ConTrol(ft, dse, dse2, dse3, dd1, dd2, dd3);
        DisPlay(ft, dse, dse2, dse3, dd1, dd2, dd3);
        if(Process(ft, dse, dse2, dse3, dd1, dd2, dd3) == -1)
            break;
